...
// Variabile globale per ricordare cosa c'era sullo schermo
int lastDrawnMode = -1;
...
// --- EFFETTO 1: INGRANAGGI (10 frames) ---
#define GEARS_FRAME_COUNT 10
static const unsigned char PROGMEM anim_gears[GEARS_FRAME_COUNT][128] = {
...
// --- EFFETTO 2: SPIRALE (Immagine fissa 72x40px) ---
...
// Mostra l'animazione ingranaggi
void displayAnimGears()
{
  ...
  if (millis() - lastFrameTime > 42)
  {
    ...
    u8g2.drawBitmap(ANIM_X, ANIM_Y, FRAME_WIDTH / 8, FRAME_HEIGHT, anim_gears[frame]);

    u8g2.sendBuffer();

    frame = (frame + 1) % GEARS_FRAME_COUNT;
  }
}

// Mostra la spirale statica (chiamata una sola volta al cambio modalità)
void displayImageSpirale()
{
  if (currentMode != lastDrawnMode)
  {
    u8g2.clearBuffer();
...
void updateDisplayState()
{
  ...
  {
  case 1:
    displayAnimGears(); // L'animazione ingranaggi
    lastDrawnMode = -1; // Reset per forzare il ridisegno quando si passa a un'immagine statica
    break;
  case 2:
    displayImageSpirale(); // L'immagine spirale statica
    lastDrawnMode = -1;    // Reset per forzare il ridisegno quando si passa a un'immagine statica
    break;
  case 3:
